...
WORKDIR /src
COPY ["SW.Webapisl.csproj", "SW.Webapisl/"]
RUN dotnet restore "SW.Webapisl/SW.Webapisl.csproj"

WORKDIR "/src/SW.Webapisl"
COPY . .
RUN dotnet build "SW.Webapisl.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "SW.Webapisl.csproj" \
...
Execute unit tests
```
    cd "SW.Webapisl/test/SW.Webapisl.Tests"
    dotnet test
```

Deploy application
```
    cd "SW.Webapisl/src/SW.Webapisl"
